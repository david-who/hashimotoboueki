...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		$options = get_desing_plus_option();
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image1')){
				$file = basename($options['slider_image1']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image1'] = '';
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image2')){
				$file = basename($options['slider_image2']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image2'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image3')){
				$file = basename($options['slider_image3']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image3'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image4')){
				$file = basename($options['slider_image4']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image4'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_slider_image5')){
				$file = basename($options['slider_image5']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['slider_image5'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_header_ad_image2')){
				$file = basename($options['header_ad_image2']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['header_ad_image2'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_header_ad_image3')){
				$file = basename($options['header_ad_image3']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['header_ad_image3'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_single_ad_image3')){
				$file = basename($options['single_ad_image3']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['single_ad_image3'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_side_ad_image2')){
				$file = basename($options['side_ad_image2']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['side_ad_image2'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_side_ad_image3')){
				$file = basename($options['side_ad_image3']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['side_ad_image3'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_footer_ad_image1')){
				$file = basename($options['footer_ad_image1']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['footer_ad_image1'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_footer_ad_image2')){
				$file = basename($options['footer_ad_image2']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['footer_ad_image2'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_footer_ad_image3')){
				$file = basename($options['footer_ad_image3']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['footer_ad_image3'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
