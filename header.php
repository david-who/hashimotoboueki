...
<div id="wrapper-<?php echo $options['site_style']; ?>">
   ...
   <?php if(!is_mobile()) { ?>
	...
	<?php if($options['header_right']=='banner') : ?>
					<!-- header banner -->
<?php
	if($options['header_ad_code3']||$options['header_ad_image3']){
		$rdm=mt_rand(1,3);
	}else if($options['header_ad_code2']||$options['header_ad_image2']){
		$rdm=mt_rand(1,2);
	}else if($options['header_ad_code1']||$options['header_ad_image1']){
		$rdm=1;
	}else{
		$rdm=0;
	}
...
<?php if($rdm!=0){ ?>
   <div id="header_banner">
    <?php if ($options['header_ad_code'.$rdm]) { ?>
     <?php echo $options['header_ad_code'.$rdm]; ?>
    <?php } else { ?>
     <a href="<?php esc_attr_e( $options['header_ad_url'.$rdm] ); ?>" class="target_blank"><img src="<?php esc_attr_e( $options['header_ad_image'.$rdm] ); ?>" alt="" title="" /></a>
    <?php }; ?>
   </div>
<?php }; ?>
					<!-- /header banner -->
